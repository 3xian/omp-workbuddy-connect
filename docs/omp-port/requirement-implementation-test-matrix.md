...
| UX-01 | `src/ui.ts`, command state | available/unavailable/not-queried render; negative/incoherent/empty-package data rejected; no stale last-good | command registration and normalized Usage report | `test/ui.test.mts` status fields; `test/credits.test.mts` genuine zero and semantic-invalid boundaries | live Billing remains M5 release evidence | M4 PASS |
| UX-02 | command orchestration | free/all/logout transitions | command/UI API contract | scope commit/rollback and unchanged credential; provider-scoped delete, failed-delete truthfulness, and post-delete refresh-failure truthfulness | live command UX remains M5 release evidence | M4 PASS |
| UX-03 | `src/credits.ts`, `src/workbuddy-api.ts`, Provider `usage` | strict parser and genuine-zero cases | normalized schema; `retainLastGoodOnFailure=false`; `validatesCredentials=false`; host `X-User-Id`; no Billing enterprise header | success→5xx, malformed/semantic-invalid/empty, timeout, slow-start nonblocking; single-account zero-request guard | live Billing and credential-error semantics remain M5 release evidence | M4 PASS |
