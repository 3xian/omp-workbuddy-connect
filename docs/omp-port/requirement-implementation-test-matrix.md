...
| HOST-05 | gate/evidence docs | requirement inventory count | strict OpenSpec validation | M0–M5 functional command sets and 19-script runner | historical dated release matrix retained | Functional regressions PASS; publication BLOCKED by AUTH-04 |
...
| AUTH-04 | `src/provider.ts` sole-row pre/post resolver; host request identity still required | durable ID/account/org comparison, optional enterprise, pre-bind projection, shared-store binding | Bearer-before-Header and per-401 Headers, but no request session in `resolveHeaders` | serial refresh/retry; cross-session retained A→B; mid-resolver row switch rejection | prior live A/B was serial | **BLOCKED — no atomic Bearer/Header request identity; no publication** |
...
| AUTH-07 | single-account cardinality and row-change guard | 0/1/2 rows; cross-session binding; active not used as request proof | `listOAuthAccounts(provider)` plus selected `getApiKey` validation | retained A→B through second request session; shared AuthStorage does not clobber binding | prior distinct A→B executions were serial | M1 cardinality PASS; publication blocked by AUTH-04 |
...
| UX-01 | `src/ui.ts`, command state | available/unavailable/not-queried compact render; model list first four plus remainder; negative/incoherent/empty-package data rejected; no stale last-good | command registration and normalized Usage report | `test/ui.test.mts` seven-line fields, long-list truncation, and masked identity; `test/credits.test.mts` genuine zero and semantic-invalid boundaries | official Billing success and live unavailable/logout transitions | M4/M5 PASS |
...
| UX-05 | command-scoped lifecycle display | masked compact detail; no WorkBuddy status line | session_start/turn_start hooks clear UI without Billing | explicit show, next-turn clear/invalidate, startup dormant, UI exceptions contained | live free/all switch, credits, login/logout, and masked identity | M4 PASS; M5 PASS |
