...
| HOST-05 | gate/evidence docs | requirement inventory count | strict OpenSpec validation | M0–M5 gate command sets and 19-script release runner | dated redacted OAuth/Chat/Refresh/Vision/Tools/Credits/main/Task/headless matrix | M0–M5 PASS; v1 signed |
...
| UX-01 | `src/ui.ts`, command state | available/unavailable/not-queried render; negative/incoherent/empty-package data rejected; no stale last-good | command registration and normalized Usage report | `test/ui.test.mts` status fields and masked identity; `test/credits.test.mts` genuine zero and semantic-invalid boundaries | official Billing success and live unavailable/logout transitions | M4/M5 PASS |
| UX-02 | command orchestration | free/all/logout transitions | command/UI API contract | scope commit/rollback and unchanged credential; provider-scoped delete, failed-delete truthfulness, and post-delete refresh-failure truthfulness | live free/all/logout and post-logout fail-closed passed | M4/M5 PASS |
| UX-03 | `src/credits.ts`, `src/workbuddy-api.ts`, Provider `usage` | strict parser and genuine-zero cases | normalized schema; `retainLastGoodOnFailure=false`; `validatesCredentials=false`; host `X-User-Id`; no Billing enterprise header | success→5xx, malformed/semantic-invalid/empty, timeout, slow-start nonblocking; single-account zero-request guard | official Billing success; controlled 5xx/timeout/slow production-adapter faults | M4/M5 PASS |
| UX-04 | `src/ui.ts` `stateGeneration` | stale-result discard | session/account/scope/model inputs checked before apply | account A→B, scope change, model leave, logout pending result, shutdown invalidation | live logout/scope/credits passed; prior live A→B retained | M4 PASS; M5 PASS |
| UX-05 | lifecycle display | visibility, masked identity, and eight-field render | session_start/turn_start hooks | next-turn switch-out clear; session startup does not await Billing; UI exceptions contained | live free/all switch, credits, login/logout, and masked identity | M4 PASS; M5 PASS |
...
- Gate record: `docs/omp-port/m4-gate.md`. Widget model changes are intentionally synchronized on the next `turn_start`; the subsequent live Billing and complete release matrix passed at M5 and are archived in `release-evidence.md`.
