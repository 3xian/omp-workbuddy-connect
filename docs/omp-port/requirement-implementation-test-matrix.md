...
| AUTH-04 | `src/provider.ts` session-selected durable-row resolver | active-row requirement, durable ID/account/org pre/post comparison, optional enterprise, pre-bind projection | production `streamSimple()` Bearer-before-Header order and per-401-attempt resolver | first request; same-row refresh; two Header passes for 401+retry; paused A plus concurrent B rejects A before transport | prior live A/B plus current real-AuthStorage contract | M1 PASS; post-merge race hardening PASS |
...
| AUTH-07 | single-account plus session-selected-row guard | 0/1/2 rows; no-active rejection; concurrent active A→B race | `listOAuthAccounts(provider, sessionId)` durable `credentialId` and `active` | retained/fresh-child A→B; concurrent B succeeds while paused A fails closed | distinct A→B retained and Task sessions | M1 PASS |
