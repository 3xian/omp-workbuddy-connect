...
| HOST-05 | gate/evidence docs | requirement inventory count | strict OpenSpec validation | M0–M4 gate command sets | M1 live authentication/Gateway chain; full release live matrix remains M5 | M0–M4 PASS; M5 NOT RUN |
...
| UX-01 | `src/ui.ts`, command state | available/unavailable/not-queried render; no stale last-good | command registration and normalized Usage report | `test/ui.test.mts` status fields; `test/credits.test.mts` success→failure | live Billing remains M5 release evidence | M4 PASS |
| UX-02 | command orchestration | free/all/logout transitions | command/UI API contract | scope commit/rollback and unchanged credential; provider-scoped delete and failed-delete behavior | live command UX remains M5 release evidence | M4 PASS |
| UX-03 | `src/credits.ts`, `src/workbuddy-api.ts`, Provider `usage` | strict parser and genuine-zero cases | normalized schema; `retainLastGoodOnFailure=false`; host `X-User-Id`; no Billing enterprise header | success→5xx, malformed, timeout, slow-start nonblocking; single-account zero-request guard | live Billing remains M5 release evidence | M4 PASS |
| UX-04 | `src/ui.ts` `stateGeneration` | stale-result discard | session/account/scope/model inputs checked before apply | account A→B, scope change, model leave, logout pending result, shutdown invalidation | live pending Billing switch remains M5 release evidence | M4 PASS |
| UX-05 | lifecycle display | visibility and eight-field render | session_start/turn_start hooks | next-turn switch-out clear; session startup does not await Billing; UI exceptions contained | interactive live switch remains M5 release evidence | M4 PASS |
| UX-06 | `ctx.hasUI` guards | no-UI branch | auth/provider/hook registration remains unconditional | headless lifecycle/status command performs zero UI access and starts no optional Billing; Task contract remains green | authenticated headless tools remain M5 release evidence | M4 PASS |
| UX-07 | `src/settings.ts` | `settings.test.mts`: public agent path/scope/0600/no-secret/atomic-failure cases | OMP `getAgentDir()` rule | isolated directory, byte-preserving failure, temp cleanup, empty-free restart; UI reads committed scope | N/A | M2 3.9 PASS; M4 PASS |
...
- UX-01/03: `src/credits.ts`, the Billing protocol in `src/workbuddy-api.ts`, and `test/credits.test.mts` cover normalized account/plan/pack data, genuine zero, success→5xx, malformed data, timeout, no stale last-good, single-account rejection, `X-User-Id`, and absence of unevidenced Billing `X-Enterprise-Id`.
- UX-01/02/04/05/06: `src/ui.ts`, `extensions/workbuddy.ts`, and `test/ui.test.mts` cover all status fields, command refresh, account/scope/model/session generation invalidation, optional UI exception containment, and headless zero-UI access.
- UX-02/04: `test/contract/model-scope-lifecycle.test.mts` and `test/contract/provider-logout.test.mts` retain transactional free/all changes, credential invariance/delete semantics, pending-Billing logout, and model-leave clearing.
- UX-03/05: `test/session-start.test.mts` now uses a real isolated OMP AuthStorage OAuth row plus registered UsageProvider and proves pending Billing does not delay `session_start`.
- UX-07: `test/settings.test.mts` retains host-directory scope persistence and secret-free settings.
- Gate record: `docs/omp-port/m4-gate.md`. Widget model changes are intentionally synchronized on the next `turn_start`; full live Billing and release matrix evidence remains M5 work.
