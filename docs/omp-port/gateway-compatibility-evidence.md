...
- `test/scope.test.mts`: actual extension hook; a request-bound WorkBuddy named choice receives a copied payload with only `tool_choice` changed, compatible string choices retain identity, and current/historical same-ID foreign Provider requests receive no replacement or mutation.
- `test/contract/before-provider-request-runtime.test.mts`: actual OMP `ExtensionRunner`; the exact request Model is exposed as `ctx.model`, and a throwing hook is reported but does not block or replace the payload.

These fixtures do not call WorkBuddy and cannot prove Gateway acceptance.

## Request-bound isolation

OMP 18.2.6 supplies the exact provider request Model as hook `ctx.model`. The compatibility hook therefore gates only on `ctx.model.provider === "workbuddy"`; payload model IDs do not participate in Provider identification, so current and historical same-ID foreign requests remain untouched. Because `ExtensionRunner` catches ordinary hook failures and continues with the original payload, active-scope, transition, and retained-model fail-closed checks live in each WorkBuddy model's `resolveHeaders`. The resolver checks before an existing resolver, after awaits, and before returning identity headers; a scope revision change or inactive ID stops the request before WorkBuddy HTTP.
...
After request routing moved from payload-ID matching to request-bound Provider identity, a fresh isolated login repeated the corrected forced named-tool case on `deepseek-v4.1-flash`: OMP displayed the selected model, executed one real `Read package.json`, and returned `DEEP_NAMED_TOOL_OK` without code `11101` or HTTP 400. `/logout workbuddy` then removed the isolated credential.
